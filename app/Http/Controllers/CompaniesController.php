...
class CompaniesController extends Controller
{
    ...
    public function postCompanies(Request $request)
    {
        ...
        if ($validator->fails()) {
			...
			$autoSeed->company_autoseed($company->company_id);
    ...
    public function putCompanies(Request $request)
    {
        if ($request->isDelete) {
    ...
    {
        $company   = tbl_company_model::find($company_id);
        $locations = tbl_unit_location_model::
    ...
    public function postLocations(Request $request, $company_id)
    {
        $add_location_rules = [
    ...
    public function putLocations(Request $request, $company_id)
    {
        if ($request->isDelete) {
    ...
    public function getEmploymenttypes($company_id)
    {
        $company           = tbl_company_model::find($company_id);
        $employement_types = tbl_employee_type_model::
            where('company_id', $company->company_id)
            ->where('active_flag', 'Y')
    ...
    public function putEmploymenttypes(Request $request, $company_id)
    {
        if ($request->isDelete) {
    ...
    public function postEmploymenttypes(Request $request, $company_id)
    {
    ...
    public function getSalarygrades($company_id)
    {
        $company       = tbl_company_model::find($company_id);
        $salary_grades = tbl_salary_grade_model::
    ...
    public function postSalarygrades(Request $request, $company_id)
    {
    ...
    public function putSalarygrades(Request $request, $company_id)
    {
        if ($request->isDelete) {
    ...
    public function getClassification($company_id)
    {
        ...
        $classifications = tbl_classification_model::
            ...
            ->orderby('class_name')
            ->get();
        return view('main/company_setup/classification'
    ...
    {
        $add_class_rules = ['class_name' => 'required'];
        $add_class_msg   = ['class_name.required' => 'Classification name is a required field.'];
        ...
        if ($validator->fails()) {
            ...
            tbl_classification_model::create([
                'class_name' => $request->class_name
                , 'active_flag' => 'Y'
                //, 'company_id' => $this->currentCompany->company_id	// 20161020 updated by Melvin Militante; Reason: Creation of class should use the company under it not the current selected company
				, 'company_id' => $company_id
                , 'created_by' => $this->currentUser->user_id
                , 'updated_by' => $this->currentUser->user_id,
            ]);
    ...
    public function putClassification(Request $request, $company_id)
    {
        if ($request->isDelete) {
            if (count($request->classification) > 0) {
                foreach ($request->classification as $class) {
                    tbl_classification_model::find($class)->update([
                        'active_flag' => 'Y'
                        , 'updated_by' => $this->currentUser->user_id,
                    ]);
                }
            ...
            $put_emptype_rules = ['put_class_name' => 'required'];
            $put_emptype_msg   = ['put_class_name.required' => 'Classification is a required field.'];
            ...
            if ($validator->fails()) {
                ...
                tbl_classification_model::find($request->put_class_id)->update([
    ...
    public function getPositions($company_id)
    {
        $company   = tbl_company_model::find($company_id);
        $positions = tbl_position_model::
            where('company_id', '=', $company_id)
            ->where('active_flag', 'Y')
            ->whereIn('business_unit_id', tbl_business_unit_model::
                    where('company_id', $company_id)
                    ->where('active_flag', 'Y')
                    ->lists('business_unit_id'))
            ->whereIn('grade_id', tbl_salary_grade_model::
                    where('company_id', $company_id)
                    ->where('active_flag', 'Y')
                    ->lists('grade_id'))
            ->whereIn('class_id', tbl_classification_model::
                    where('company_id', $company_id)
                    ->where('active_flag', 'Y')
                    ->lists('class_id'))
    ...
    public function postPositions(Request $request, $company_id)
    {
        $add_position_rules = [
    ...
    public function putPositions(Request $request, $company_id)
    {
        if ($request->isDelete) {
    ...
    public function getChangecompany()
    {
    ...
    public function getChangecompany2($company_id)
    {
    ...
    public function postChangecompany(Request $request)
    {
