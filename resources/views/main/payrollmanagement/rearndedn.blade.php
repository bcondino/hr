				...
				<table id="rearndedn" class="uk-table uk-table-hover uk-table-striped payroll--table">
				    ...
				    <tbody>
				    	@foreach($rearndedns as $rearndedn)
							<tr>
					            <td> <input type="checkbox" id="select_all" class="chk-earndedns" name="earndedns[]" value="{{ $rearndedn->recur_earndedn_id }}"/></td>
	...
	$(document).ready(
		function() {
			...
			$("#btn-del").click(function(){
				$(".chk-earndedns:checked").each(function(){
					$('#div-del-chk-rearndedn').append('<input type="hidden" name="earndedns[]" value="'+ $(this).val() +'" />');
				});
			});
