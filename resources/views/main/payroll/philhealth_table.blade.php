...
@section('title', 'Payroll: Philhealth')
...
<div class="header--title_container">
	<div class="uk-container uk-container-center">
		<div class="container-title">
			<h1 class="page-title"><span class="uk-icon uk-icon-file-text"></span> <strong>Philhealth Table</strong> </a></h1>
		</div>
	</div>
</div>
	...
	<div class="uk-grid main">
		<div class="uk-width-1-4">
			<ul class="uk-nav uk-nav-side uk-nav-parent-icon" data-uk-nav>
				...
				<li class="uk-active"><a href="{{ url('payroll/philhealthtable') }}">Philhealth Table</a></li>
		...
		<div class="uk-width-3-4" >
			<article class="uk-article">
				...
				<!-- philhealth -->
				<table id="philhealth_table" class="uk-table uk-table-hover uk-table-striped payroll--table">
					<thead class="payroll--table_header">
						<tr> 
							<th><input type="checkbox" name="select_all" id="select_all" value="1" ></th>
				            <th>Monthly <br />Salary Bracket</th>
					...
					<tbody>
						@foreach($philhealths as $philhealth)
							<tr>
					            <td><input type="checkbox" id="select_all" class="chk-philhealth_tbl" name="philhealth[]" value="{{$philhealth->philhealth_id}}"/></td>
					            <td><a class="btn-edit" data-uk-modal="{target:'#edit'}"
									data-edit_philhealth_id="{{ $philhealth->philhealth_id }}"
									data-edit_msb="{{ $philhealth->msb }}"
								data-edit_range_from="{{ $philhealth->range_from }}"
								data-edit_range_to="{{ $philhealth->range_to}}"
								data-edit_salary_base="{{ $philhealth->salary_base }}"
								data-edit_ee_cont="{{ $philhealth->ee_cont}}"
								data-edit_er_cont="{{ $philhealth->er_cont}}"
								>{{ $philhealth->msb }}</a></td>
					            <td>{{ number_format($philhealth->range_from, 2, ".", ",") }}</td>
					            <td>{{ number_format($philhealth->range_to, 2, ".", ",") }}</td>
					            <td>{{ number_format($philhealth->salary_base, 2, ".", ",") }}</td>
					            <td>{{ number_format($philhealth->ee_cont, 2, ".", ",") }}</td>
					            <td>{{ number_format($philhealth->er_cont, 2, ".", ",") }}</td>
							</tr>
						@endforeach
					</tbody>
				</table> <!-- tax exemption -->
...
<div id="add" class="uk-modal">
    <div class="uk-modal-dialog">
    	...
    	<div class="uk-modal-header"><span class="uk-icon-plus-circle"></span>Add Philhealth Table</div>
		<!-- alerts -->
			...
			@endif
			<!-- end alerts -->
        <form class="uk-form uk-form-horizontal" method="post" action="{{url('payroll/philhealthtable')}}">
        	{{csrf_field()}}
		    <fieldset>
		    	<div class="uk-form-row">
		        	<label class="uk-form-label"><span class="uk-text-danger">*</span> Monthy Salary Bracket</label>
		        	<div class="uk-form-controls">
		        		<input type="text" class="form-control" name="msb" value="{{old('range_from')}}">
		        	</div>
		        </div>
		        <div class="uk-form-row">
		        ...
		        <div class="uk-form-row">
		        	<label class="uk-form-label"><span class="uk-text-danger">*</span> Salary Base</label>
		        	<div class="uk-form-controls">
		        		<input type="text" class="form-control" name="salary_base" value="{{old('salary_base')}}">
		        	</div>
		        </div>
		        <div class="uk-form-row">
...
<div id="edit" class="uk-modal">
    <div class="uk-modal-dialog">
    	<button class="uk-modal-close uk-close"></button>
    	<div class="uk-modal-header"><span class="uk-icon-edit"></span>Edit Philhealth Table</div>
		<!-- alerts -->
		...
		<input type="hidden" name="philhealth" value="{{old('philhealth')}}"/>
		    <fieldset>
		   <div class="uk-form-row">
		        	<label class="uk-form-label"><span class="uk-text-danger">*</span> Monthy Salary Bracket</label>
		        	<div class="uk-form-controls">
		        		<input type="text" class="form-control" name="msb" id="edit_msb" value="{{old('msb')}}">
		        	</div>
		        </div>
		        <div class="uk-form-row">
		        	<label class="uk-form-label"><span class="uk-text-danger">*</span> Range From</label>
		        ...
		        <div class="uk-form-row">
		        	<label class="uk-form-label"><span class="uk-text-danger">*</span> Salary Base</label>
		        	<div class="uk-form-controls">
		        		<input type="text" class="form-control" name="salary_base" id="edit_salary_base" value="{{old('salary_base')}}">
		        	</div>
		        </div>
		        <div class="uk-form-row">
		        	<label class="uk-form-label"><span class="uk-text-danger">*</span> Employee Contribution</label>
		        	<div class="uk-form-controls">
		        		<input type="text" class="form-control" name="ee_cont" id="edit_ee_cont" value="{{old('ee_cont')}}">
		        	</div>
		        </div>
		        <div class="uk-form-row">
		        	<label class="uk-form-label"><span class="uk-text-danger">*</span> Employer Contribution</label>
		        	<div class="uk-form-controls">
		        		<input type="text" class="form-control" name="er_cont" id="edit_er_cont" value="{{old('er_cont')}}">
		        	</div>
		        </div>
		    </fieldset>
		    <div class="uk-modal-footer uk-text-right form-buttons">
...
<div id="delete" class="uk-modal">
    <div class="uk-modal-dialog">
    	...
    	<form method="post" action="{{url('payroll/philhealthtable')}}" >
    		{{ csrf_field() }}
    		{{ Form::hidden('_method', 'put') }}
    		{{ Form::hidden('isDelete', '1') }}
	    	<div id="div-del-chk-philhealth_tbl">
	    	</div>
		    <div class="uk-modal-footer uk-text-right form-buttons">
	...
	$(document).ready(
		function() {
			...
			$("#btn-del").click(function(){
					$(".chk-philhealth_tbl:checked").each(function(){
					$('#div-del-chk-philhealth_tbl').append('<input type="hidden" name="philhealth_tbl[]" value="'+ $(this).val() +'" />');
					});
				});
			...
			$(".btn-edit").click(function(){
				$("input[name|=philhealth]").val( $(this).attr('data-edit_philhealth_id') );
				$("#edit_msb").val( $(this).attr('data-edit_msb') );
				$("#edit_range_from").val( $(this).attr('data-edit_range_from') );
				$("#edit_range_to").val( $(this).attr('data-edit_range_to') );
				$("#edit_salary_base").val( $(this).attr('data-edit_salary_base') );
				$("#edit_ee_cont").val( $(this).attr('data-edit_ee_cont') );
				$("#edit_er_cont").val( $(this).attr('data-edit_er_cont') );
				});
			
				var dataTable = $('#philhealth_table').DataTable({
					order: [],
