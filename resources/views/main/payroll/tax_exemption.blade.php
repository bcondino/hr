	...
	<div class="uk-grid main">
		<div class="uk-width-1-4">
			<ul class="uk-nav uk-nav-side uk-nav-parent-icon" data-uk-nav>
				<li class="uk-active"><a href="{{ url('payroll/taxexemption') }}">Tax Exemption</a></li>
			...
			<article class="uk-article">
				...
				<table id="tax_exemption" class="uk-table uk-table-hover uk-table-striped payroll--table">
					...
					@foreach($tax_codes as $tax_code)
						<tr>
							<td><input type="checkbox" id="select_all" class="chk-tax_exemption" name="tax_exemption[]" value="{{$tax_code->tax_code_id}}"/></td>
							<td><a class="btn-edit" data-uk-modal="{target:'#edit'}"
									data-edit_tax_code_id="{{ $tax_code->tax_code_id }}"
									data-edit_tax_code="{{ $tax_code->tax_code }}"
									data-edit_description="{{ $tax_code->description }}"
								data-edit_exemption_amount="{{ $tax_code->exemption_amount }}">
															
							{{ $tax_code->tax_code }}</a></td>
							<td>{{ $tax_code->description }}</td>
							<td>{{ number_format($tax_code->exemption_amount, 2, ".", ",") }}</td>
						</tr>
						@endforeach
					</tbody>
				</table> <!-- tax exemption -->
...
<div id="add" class="uk-modal">
    <div class="uk-modal-dialog">
        ...
        <form class="uk-form uk-form-horizontal" method="post" action="{{url('payroll/taxexemption')}}" >
        	{{csrf_field()}}
		    <fieldset>
		        ...
		        <div class="uk-form-row">
		        	...
		        	<div class="uk-form-controls">
		        		<input type="text" class="form-control" name="exemption_amount" value="{{old('exemption_amount')}}">
		        	</div>
		        </div>
...
<div id="edit" class="uk-modal">
    <div class="uk-modal-dialog">
        ...
        <form class="uk-form uk-form-horizontal" method="post" action="{{ url('payroll/taxexemption') }}">
			...
			<input type="hidden" name="tax_codes" value="{{old('tax_codes')}}"/>
		    <fieldset>
		        <div class="uk-form-row">
		        	<label class="uk-form-label"><span class="uk-text-danger">*</span> Tax Code</label>
		        	<div class="uk-form-controls">
		        		<input type="text" class="form-control" name="tax_code" id="edit_tax_code" placeholder="" value="{{old('tax_code')}}">
		        	</div>
		        </div>
		        <div class="uk-form-row">
		        	<label class="uk-form-label"><span class="uk-text-danger">*</span> Description</label>
		        ...
		        <div class="uk-form-row">
		        	<label class="uk-form-label"><span class="uk-text-danger">*</span> Exemption Amount</label>
		        	<div class="uk-form-controls">
		        		<input type="text" class="form-control" name="exemption_amount" id="edit_exemption_amount" placeholder="" value="{{old('exemption_amount')}}">
		        	</div>
		        </div>
...
<div id="delete" class="uk-modal">
    <div class="uk-modal-dialog">
    	...
    	<form method="post" action="{{url('payroll/taxexemption')}}" >
    		{{ csrf_field() }}
    		{{ Form::hidden('_method', 'put') }}
