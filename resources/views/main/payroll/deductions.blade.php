...
@section('title', 'Payroll: Details Deductions')
...
<div class="header--title_container">
	<div class="uk-container uk-container-center">
		<div class="container-title">
			<h1 class="page-title"><span class="uk-icon uk-icon-file-text"></span> <strong>Details Deductions</strong> </a></h1>
		</div>
	</div>
</div>
	...
	<div class="uk-grid main">
		...
		<div class="uk-width-3-4" >
			...
			<!-- buttons -->
				...
				<!-- payroll Deductions -->
				<table id="deductions" class="uk-table uk-table-hover uk-table-striped payroll--table">
					...
					<tbody>
						@foreach($deductions as $deduction)
							<tr>
								<td>
									@if($deduction->created_by == null)
										<input type="checkbox" disabled>
									@else 
										<input type="checkbox" class="chk-deductions" name="deductions_tbl[]" value="{{ $deduction->payroll_element_id }}"/>
									@endif
								</td>
								<td><a class="btn-edit" data-uk-modal="{target:'#edit'}"
									data-edit_payroll_element_id="{{ $deduction->payroll_element_id}}"
									data-edit_description="{{ $deduction->description}}"
									data-edit_tran_code="{{ $deduction->tran_code}}"
									data-edit_taxable_flag="{{ $deduction->taxable_flag}}"
									data-edit_regular_flag="{{ $deduction->regular_flag}}"
									data-edit_tax_exempt_flag="{{ $deduction->tax_exempt_flag}}"
									data-edit_fb_tax_flag="{{ $deduction->fb_tax_flag}}"
									data-edit_deminimis_flag="{{ $deduction->deminimis_flag}}"
									data-edit_sss_flag="{{ $deduction->sss_flag}}"
									data-edit_pagibig_flag="{{ $deduction->pagibig_flag}}"
									data-edit_philhealth_flag="{{ $deduction->philhealth_flag}}"
									data-edit_loan_flag="{{ $deduction->loan_flag}}"
									data-edit_show_payslip="{{ $deduction->show_payslip}}"
								>{{ ucwords($deduction->description) }}</a></td>
								<td> {{ $deduction->taxable_flag == 'Y' ? 'Yes' : 'No' }} </td>
								<td> {{ $deduction->tax_exempt_flag == 'Y' ? 'Yes' : 'No' }} </td>
								<td> {{ $deduction->fb_tax_flag == 'Y' ? 'Yes' : 'No' }} </td>
								<td> {{ $deduction->sss_flag == 'Y' ? 'Yes' : 'No' }} </td>
								<td> {{ $deduction->gsis_flag == 'Y' ? 'Yes' : 'No' }} </td>
								<td> {{ $deduction->pagibig_flag == 'Y' ? 'Yes' : 'No' }} </td>
								<td> {{ $deduction->philhealth == 'Y' ? 'Yes' : 'No' }} </td>
								<td> {{ $deduction->loan_flag == 'Y' ? 'Yes' : 'No' }} </td>
...
<div id="add" class="uk-modal">
    <div class="uk-modal-dialog modal-wide">
    	...
    	<div class="uk-modal-header"><span class="uk-icon-plus-circle"></span>Add Payroll Detail: Deductions</div>
			<!-- alerts -->
        ...
        <form class="uk-form uk-form-horizontal" method="post" action="{{url('payroll/deductions')}}" >
        	{{csrf_field()}}
        	<div class="uk-grid">
				    ...
				    <fieldset class="uk-form-horizontal">
				    	<div class="uk-form-row">
							...
							<div class="uk-form-controls">
					        	<input type="text" class="form-control" name="entry_type" value="Debit" disabled>
							</div>
				        </div>
				        <div class="uk-form-row">
...
<div id="edit" class="uk-modal">
    <div class="uk-modal-dialog modal-wide">
    	<button class="uk-modal-close uk-close"></button>
    	<div class="uk-modal-header"><span class="uk-icon-edit"></span>Edit Payroll Detail: Deductions</div>
			<!-- alerts -->
		...
		<input type="hidden" name="deduction_s" value="{{ old('deduction_s') }}" />		   
        	<div class="uk-grid">
        		<div class="uk-width-1-2">
				    <fieldset class="uk-form-horizontal">
				    	<div class="uk-form-row">
...
<div id="delete" class="uk-modal">
    <div class="uk-modal-dialog">
    	...
    	<form method="post" action="{{ url('payroll/deductions') }}">
    		{{ csrf_field() }}
    		{{ Form::hidden('_method', 'put') }}
    		{{ Form::hidden('isDelete', '1') }}
	    	<div id="div-del-chk-deductions">
	    	</div>
		    <div class="uk-modal-footer uk-text-right form-buttons">
	...
	$(document).ready(
		function() {
			$("#btn-del").click(function(){
					$(".chk-deductions:checked").each(function(){
					$('#div-del-chk-deductions').append('<input type="hidden" name="deductions_tbl[]" value="'+ $(this).val() +'" />');
					});
				});
			...
			$(".btn-edit").click(function(){
				...
				$("input[name|=deduction_s]").val( $(this).attr('data-edit_payroll_element_id') );
			...
			var dataTable = $('#deductions').DataTable({
				order: [],
