...
<div class="header--title_container">
	<div class="uk-container uk-container-center">
		<div class="container-title">
			<h1 class="page-title"><span class="uk-icon uk-icon-file-text"></span> <strong> SSS Table </strong></h1>
		</div>
	</div>
</div>
	...
	<div class="uk-grid main">
		<div class="uk-width-1-4">
			<ul class="uk-nav uk-nav-side uk-nav-parent-icon" data-uk-nav>
				...
				<li class="uk-active"><a href="{{ url('payroll/ssstable') }}">SSS Table</a></li>
		...
		<div class="uk-width-3-4" >
			<article class="uk-article">
				...
				<!-- sss  -->
				<table id="sss_table" class="uk-table uk-table-hover uk-table-striped payroll--table">
					...
					<tbody>
						@foreach($ssss as $sss)
					        <tr>
					             <td><input type="checkbox" id="select_all" class="chk-sss_tbl" name="sss[]" value="{{$sss->sss_id}}"/></td>
					            <td><a class="btn-edit" data-uk-modal="{target:'#edit'}"
								data-edit_sss_id="{{ $sss->sss_id }}"
								data-edit_range_from="{{ $sss->range_from }}"
								data-edit_range_to="{{ $sss->range_to}}"
								data-edit_ee_sss_cont="{{ $sss->ee_sss_cont}}"
								data-edit_er_sss_cont="{{ $sss->er_sss_cont}}"
								data-edit_er_ec_cont="{{ $sss->er_ec_cont}}"
								>{{ number_format($sss->range_from, 2, ".", ",") }}</a></td>
					            <td>{{ number_format($sss->range_to, 2, ".", ",") }}</td>
					            <td>{{ number_format($sss->ee_sss_cont, 2, ".", ",") }}</td>
					            <td>{{ number_format($sss->er_sss_cont, 2, ".", ",") }}</td>
					            <td>{{ number_format($sss->er_ec_cont, 2, ".", ",") }}</td>
					            <td>{{ number_format(array_sum([$sss->ee_sss_cont, $sss->er_sss_cont, $sss->er_ec_cont]), 2, ".", ",") }}</td>
					        </tr>
						@endforeach
					</tbody>
				</table> <!-- tax exemption -->
...
<div id="add" class="uk-modal">
    <div class="uk-modal-dialog">
    	...
    	<div class="uk-modal-header"><span class="uk-icon-plus-circle"></span>Add SSS Table</div>
		<!-- alerts -->
		...
		@endif
			<!-- end alerts -->
        <form class="uk-form uk-form-horizontal" method="post" action="{{ url('payroll/ssstable') }}">
        	{{csrf_field()}}
		    <fieldset>
		        ...
		        <div class="uk-form-row">
		        	...
		        	<div class="uk-form-controls">
		        		<input type="text" class="form-control" name="ee_sss_cont" value="{{old('ee_sss_cont')}}" >
		        	</div>
		        </div>
		        <div class="uk-form-row">
		        	<label class="uk-form-label"><span class="uk-text-danger">*</span> Employer Contribution</label>
		        	<div class="uk-form-controls">
		        		<input type="text" class="form-control" name="er_sss_cont" value="{{old('er_sss_cont')}}" >
		        	</div>
		        </div>
		        <div class="uk-form-row">
		        	<label class="uk-form-label"><span class="uk-text-danger">*</span> Employer EC Contribution</label>
		        	<div class="uk-form-controls">
		        		<input type="text" class="form-control" name="er_ec_cont" value="{{old('er_ec_cont')}}" >
		        	</div>
		        </div>
...
<div id="edit" class="uk-modal">
    <div class="uk-modal-dialog">
    	<button class="uk-modal-close uk-close"></button>
    	<div class="uk-modal-header"><span class="uk-icon-edit"></span>Edit SSS Table</div>
			<!-- alerts -->
		...
		<input type="hidden" name="sss" value="{{old('sss')}}" />
		    <fieldset>
		        <fieldset>
		        <div class="uk-form-row">
		        	<label class="uk-form-label"><span class="uk-text-danger">*</span> Range From</label>
		        ...
		        <div class="uk-form-row">
		        	<label class="uk-form-label"><span class="uk-text-danger">*</span> Employee Contribution</label>
		        	<div class="uk-form-controls">
		        		<input type="text" class="form-control" name="ee_sss_cont" id="edit_ee_sss_cont" placeholder="" value="{{old('ee_sss_cont')}}">
		        	</div>
		        </div>
		        <div class="uk-form-row">
		        	<label class="uk-form-label"><span class="uk-text-danger">*</span> Employer Contribution</label>
		        	<div class="uk-form-controls">
		        		<input type="text" class="form-control" name="er_sss_cont" id="edit_er_sss_cont" placeholder="" value="{{old('er_sss_cont')}}">
		        	</div>
		        </div>
		        <div class="uk-form-row">
		        	<label class="uk-form-label"><span class="uk-text-danger">*</span> Employer EC Contribution</label>
		        	<div class="uk-form-controls">
		        		<input type="text" class="form-control" name="er_ec_cont" id="edit_er_ec_cont" placeholder="" value="{{old('er_ec_cont')}}">
		        	</div>
		        </div>
		    </fieldset>
		    <div class="uk-modal-footer uk-text-right form-buttons">
...
<div id="delete" class="uk-modal">
    <div class="uk-modal-dialog">
    	...
    	<form method="post" action="{{url('payroll/ssstable')}}">
    		{{ csrf_field() }}
    		{{ Form::hidden('_method', 'put') }}
    		{{ Form::hidden('isDelete', '1') }}
	    	<div id="div-del-chk-sss_tbl">
	    	</div>
		    <div class="uk-modal-footer uk-text-right form-buttons">
	...
	$(document).ready(
		function() {
				...
				$("#btn-del").click(function(){
					$(".chk-sss_tbl:checked").each(function(){
					$('#div-del-chk-sss_tbl').append('<input type="hidden" name="sss_table[]" value="'+ $(this).val() +'" />');
					});
				});
			...
			$(".btn-edit").click(function(){
				...
				$("input[name|=sss]").val( $(this).attr('data-edit_sss_id') );
				$("#edit_range_from").val( $(this).attr('data-edit_range_from') );
				$("#edit_range_to").val( $(this).attr('data-edit_range_to') );
				$("#edit_ee_sss_cont").val( $(this).attr('data-edit_ee_sss_cont') );
				$("#edit_er_sss_cont").val( $(this).attr('data-edit_er_sss_cont') );
				$("#edit_er_ec_cont").val( $(this).attr('data-edit_er_ec_cont') );
				});
							
				var dataTable = $('#sss_table').DataTable({
					order: [],
